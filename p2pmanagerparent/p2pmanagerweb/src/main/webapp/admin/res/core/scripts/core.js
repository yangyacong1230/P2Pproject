...
var utils = {
    ...
    toJsonString: function (obj) {
        ...
        if (obj.constructor == Array) return '[' + arr.join() + ']';
    ...
    copy: function (txt) {
        if (txt) {
            try {
                ...
                prompt('请使用Ctrl+C或鼠标右键点击复制以下内容：', txt);
            }
        }
        return false;
    }
...
(function ($) {
    ...
    form = {
        ...
        //表单验证绑定
        validBind: function (fm, fn) {
            var that = this;
            fm = $(fm).submit(function () {
                return that.valid(fm, fn);
            });
            fm.find(':input').filter('[data-vtype]').each(function (i, n) {
                $(n).blur(function () {
                    that._checkInput(n, function (msg) {
                        if (fn) fn(msg);
                    });
                });
            });
        ...
        valid: function (fm, fn) {
            ...
            if (!pass) {                
                window.ui.scroll(parseInt($(firstInput).offset().top - 80), function () {
                    firstInput.focus();
                });
            }
    ...
    $.fn.mask = utils.fn({
        init: function (ops) {
            ...
            return this.each(function (i, n) {                
                ...
                n = $(n);
                var div = $('<div class="ui-mask" style="position:absolute;z-index:' + ops.zIndex + ';background-color:' + ops.bgColor + ';display:none;"></div>').opacity(ops.opacity);
...
//tips
(function ($) {
    $.fn.tips = function (type, msg, callback) {
        return this.each(function (i, n) {
            ...
            n = $(n);
            var pos = n.offset();
            var eleW = n.width(), domW = dom.fullSize().width;
            if (eleW > domW) {
                pos.left += ((eleW - domW) / 2);
            }
            dom.css({ left: pos.left, top: -50 });
            dom.animate({ top: 15 }, 'fast', function () {
                setTimeout(function () {
                    dom.fadeOut();
    ...
    $.fn.dialog = utils.fn({
        init: function (ops) {
            ...
            return this.each(function (i, n) {
                ...
                var closeBth = ops.closeShow ? '<a class="dialog-close iconfont fs18" title="关闭" href="javascript:">&#xe68a;</a>' : '';
                var dom = $('<div class="ui-dialog">' +
                    (ops.title ? '<div class="dialog-title" unselectable="on" onselectstart="return false;">' + closeBth + '<h1>' + ops.title + '</h1>' + '</div>' : closeBth) +
                ...
                //设置内容高度
                var body = dom.find('.dialog-body');
                var header = dom.find('.dialog-title').height();
                ...
                //事件
                dom.find('.dialog-close').click(function () {
                    $(n).dialog('destroy');
                });
            ...
            return this.each(function (i, n) {
                var dom = n.calendar_dom = $('<div class="ui-calendar">' +
                                '<div class="calendar-title">' +
                                    '<a href="javascript:" class="calendar-nextmonth">>></a>' +
                                    '<a href="javascript:" class="calendar-prevmonth"><<</a>' +
                                ...
                                '<div class="calendar-foot">' +
                                    '<input type="button" class="ui-calendarClear c-important" value="清空" /> ' +
                                    '<input type="button" class="ui-calendarToday c-obvious" value="今天" /> ' +
                                    '<input type="button" class="ui-calendarConfirm c-priority" value="确定" />' +
                                '</div>' +
                ...
                dom.find('.ui-calendarToday').click(function () {
                    $(n).val(utils.dateToString(new Date(), ops.format));
                    hideDom();
                });

                //确定
                dom.find('.ui-calendarConfirm').click(function () {
                    $(n).val(utils.dateToString(setTime(n.calendar_date), ops.format));
                    hideDom();
                });
                ...
                dom.find('.calendar-day').click(function (event) {
                    if (event.target.tagName == 'A') {
                        ...
                        $(n).val(utils.dateToString(setTime(date), ops.format));
                        hideDom();
                    }
                });
            ...
            return this.each(function (i, n) {
                dom = '<div class="ui-slider" unselectable="on" onselectstart="return false;">' +
                      '<div class="slider-background" unselectable="on" onselectstart="return false;">' +
                          ...
                          '<div class="slider-changebtn iconfont" unselectable="on" onselectstart="return false;">&#xe6a1;</div>' +
                       '</div>' +
                       '<div class="slider-btn" unselectable="on" onselectstart="return false;" >' +
                          '<a href="javascript:void(0);" unselectable="on" onselectstart="return false;" class="slider-increase iconfont">&#xe631;</a>' +
                          '<a href="javascript:void(0);" unselectable="on" onselectstart="return false;" class="slider-cutback iconfont">&#xe621;</a>' +
                       '</div>' +
                   '</div>';
                $(n).before(dom).slider('play');
                ...
                function changeWidht(num, win) {
                    ...
                    $(n).prev().find('.slider-changebtn').css('left', _num + "%");
                    $(n).prev().find('.slider-fillingbar').css('width', _num + "%");
                };
                ...
                //绑定增加按钮事件
                $(n).prev().find('.slider-increase').on('click', function () {
                    var _num = Math.round(handNull($(n)) * n.slider_data.flagNum);
                    _num = setpSize(_num, "+");
                    $(n).val(dispalyNum(_num, n.slider_data.flagNum)).trigger('blur');
                    changeWidht(_num);
                });
                $(n).prev().find('.slider-cutback').on('click', function () {
                    var _num = handNull($(n)) * n.slider_data.flagNum;
                    _num = setpSize(_num, "-");
                    $(n).val(dispalyNum(_num, n.slider_data.flagNum)).trigger('blur');
                    changeWidht(_num);
                });
                $(n).prev().find('.slider-background').on('mousedown', function () {
                    var thisWidht = $(this).width();
                    var moveSlider = function (e) {
                        var eleft = $(n).prev().offset().left;
        ...
        play: function () {
            return this.each(function (i, n) {
                var _width = n.slider_data.width,
                    ...
                    $this = $(n);
                $this.prev().css('width', _width + 'px');
                $this.prev().find('.slider-background').css('width', (_width - 50) + 'px');
                $this.prev().find('.slider-fillingbar').css('width', _starterWidth + '%');
                $this.prev().find('.slider-changebtn').css('left', _starterWidth + '%');
                $this.css('margin-left', (_width + 5) + 'px').val(dispalyNum(n.slider_data.starter, n.slider_data.flagNum)).attr({
